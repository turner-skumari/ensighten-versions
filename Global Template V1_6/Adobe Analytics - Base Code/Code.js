...
s.cookieDomainPeriods = tAAC.cookieDomainPeriods; 
...
s.doPlugins = function (s) {
    ...
    if(s.pageName || s.contextData.pageName) {
        s.getPercentPageViewed((s.pageName || s.contextData.pageName),false);
    }
    if(s._ppvPreviousPage) {
        s.contextData.percentpageviewed = s._ppvHighestPercentViewed;
    }
};
...
window.tdi_s_gi = tdi_s_gi;window.tdi_s_pgicq = tdi_s_pgicq;
 window.AppMeasurement = AppMeasurement;
}]);Bootstrapper.AF.push(['push', 'SiteCatalyst', 'ns', 'tdi_s']);Bootstrapper.AF.push(['set', 'SiteCatalyst', 'clearData', function() { if (this.ns) {for (var i = 0; i < this.ns.length; i++) {var sObj = this.ns[i]; Bootstrapper.AF.push(['set', sObj, 'clearData', function() {this.pre = [];}]);}}}]);
